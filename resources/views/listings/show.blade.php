...
<div class="mx-4">
    <x-card class="p-10">
        <div class="flex flex-col items-center justify-center text-center">
            <img class="w-48 mr-6 mb-6" src="{{($gig->logo)? asset("storage/" . $gig->logo) : asset('imgs/no-image.png') }}" alt="" />

            <h3 class="text-2xl mb-2">{{$gig->title}}</h3>
            <div class="text-xl font-bold mb-4">{{$gig->company}}
            <div class="w-1/3 mx-auto mt-2">
                <x-list-tags :csvTags="$gig->tags" />
            </div>
        ...
        <x-card class="mt-4 p-2 flex space-x-6 w-1/5 mx-auto flex justify-between">
            ...
            <form method="post" action="/listings/{{$gig->id}}">
                @csrf
                @method("DELETE")
